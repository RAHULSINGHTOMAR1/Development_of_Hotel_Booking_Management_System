...
def check_booking(uid, room_count, start_date, end_date):
    # Query bookings for the specified hotel
    qs = HotelBooking.objects.filter(hotel__uid=uid)

    # Filter to find overlapping bookings
    qs = qs.filter(
        Q(start_date__gte=start_date, end_date__lte=end_date) |  # Booking fully within requested range
        Q(start_date__lte=start_date, end_date__gte=end_date)    # Booking overlapping requested range
    )

    # Check if the number of bookings is greater or equal to room count
